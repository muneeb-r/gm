  ...
  const fetchStudents = async () => {
    if (queries.rns) {
      ...
      try {
        ...
        for (const rn of RNs) {
          if (rn) {
            const res = await axios.get("/api/student/get?rn=" + parseInt(rn)+'&campus='+localStorage.getItem('campus'));
            if (res.data) {
              studs.push(res.data);
            }
          }
  ...
  return error ? (
    <div className="w-full h-screen flex justify-center items-center flex-col">
      <h1 className="text-red-500 text-lg">
        Oops! Something went wrong. Please try again by changing some
        information.
      </h1>
      <button className="base__button mt-3" onClick={() => router.back()}>
        Go back
      </button>
    </div>
  ) : (
    <>
      ...
      <div className="flex w-full fixed top-0 h-screen left-0 bg-white justify-center">
        <div
          className="flex border border-gray-200 p-3 absolute"
          style={{
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3 pb-2 mb-1 border-b border-gray-200">
              ...
              <h1 className="text-lg font-semibold">
                GM School And Colleges Of Sciences
              </h1>
            </div>
        ...
        {showSliders && (
          <div className="flex mt-64 absolute bottom-0 gap-3 items-center">
...
const Student = ({ student, gap }) => {
  ...
  useEffect(() => {
    const fetchFe = async () => {
      const res = await axios.get(
        `/api/studentfee/getOne?studentId=${student._id}&getLast=true`
      );
      if (res.data) {
        setFee(res.data);
      } else {
  ...
  return (
    <div
      className={`flex justify-between p-1 py-2 border-b border-gray-200 items-center`}
      style={{ gap: `${gap}px` }}
    >
      ...
      <div className="flex flex-col items-center">
        ...
        {Object.keys(fee).length > 0 && (
          <div className="text-md text-gray-600 text-sm">
            Rs.
            {fee?.remainings
              ? new Intl.NumberFormat().format(fee?.remainings)
              : 0}
            /-
          </div>
        )}
      ...
      <div className="flex flex-col items-end">
        <div className="font-semibold text-sm">
          {getMonthName(fee.month?.split("-")[1])}
        </div>
        <div className="text-md text-gray-600 text-sm">
          Rs.
          {fee?.feeamount ? new Intl.NumberFormat().format(fee?.feeamount) : 0}
          /-
        </div>
      </div>
    </div>
