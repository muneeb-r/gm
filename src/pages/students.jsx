...
const Students = ({}) => {
  ...
  const fetchStudents = async () => {
    ...
    try {
      res = await axios.get(
        `/api/student/getall?campus=${localStorage.getItem(
          "campus"
        )}&items=${items}`
      );
  ...
  const fetchMore = async () => {
    ...
    try {
      ...
      if (filterType.type === "Text") {
        res = await axios.get(
          `/api/student/getall?byText=true&text=${
            filterType.data.text
          }&campus=${localStorage.getItem(
            "campus"
          )}&page=${currentPage}&items=${items}`
        );
      } else if (filterType.type === "Class") {
        res = await axios.get(
          `/api/student/getall?byClass=true&class=${fclass}&session=${fsession}&campus=${localStorage.getItem(
            "campus"
          )}&page=${currentPage}&items=${items}`
        );
      } else {
  ...
  const handleTextSearch = async (value) => {
    ...
    try {
      const res = await axios.get(
        `/api/student/getall?byText=true&text=${value}&campus=${localStorage.getItem(
          "campus"
        )}`
      );
      setStudents(res.data.students);
    ...
    if (name === stu.name.trim()) {
      try {
        const res = await axios.delete(
          "/api/student/delete?studentId=" + stu._id
        );
        if (res.data) {
          try {
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="flex mb-3 justify-between items-center">
              ...
              <div>
                <div
                  ...
                  className="w-8 h-8 font-bold rounded-full flex justify-center items-center bg-gray-50 hover:scale-125 active:rotate-180 transition-all duration-300 ease-in-out cursor-pointer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                ...
                <div className="mx-auto max-w-screen-xl">
                  <div className="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                      ...
                      <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <div className="flex gap-3 items-center flex-wrap">
                          <select
                            ...
                            value={fclass}
                            className="base__select px-3 bg-white"
                          >
                          ...
                          <span className="ml-2 flex gap-1 items-center">
                            ...
                            <select
                              ...
                              className="base__select pl-2 pr-2"
                            >
                      ...
                      <table className="w-full text-sm text-left text-gray-500 ">
                        <thead className="text-xs text-gray-900 uppercase bg-gray-200">
                          <tr>
                            ...
                            <th className="px-6 py-3 font-bold">Father Name</th>
                            <th className="px-6 py-3 font-bold">Class</th>
                            <th className="px-6 py-3 font-bold">Medium</th>
                            <th className="px-6 py-3 font-bold">R.No</th>
                            <th className="px-1 py-3 font-bold"></th>
                          </tr>
                        </thead>
                        <tbody>
                          {students?.map((stu, i) => (
                    ...
                    >
                      <button
                        onClick={fetchMore}
                        disabled={currentPage >= total?.pages || isLoading}
                        className="base__button px-3"
                      >
                        Show more
                      </button>
                      <p className="text-gray-600">
                        Showing <b>{currentPage}</b> of <b>{total.pages}</b>{" "}
                        pages
                      </p>
                    </nav>
                  </div>
        ...
        {Object.keys(feeStudent).length > 0 && (
          <AddFee feeStudent={feeStudent} setFeeStudent={setFeeStudent} />
        )}
      </main>
    </>
  );
...
const StudentRow = ({
  ...
  return (
    <>
      <tr
        className="bg-white border-b hover:bg-gray-50 animate-slow"
        title={student.name}
      >
        <td className="px-4 py-4">{i + 1}</td>
        <th
          scope="row"
          className="px-6 py-4 text-gray-900 whitespace-nowrap flex items-center font-semibold"
        >
          <Image
            className="h-10 w-10 mr-2 rounded object-cover"
            width={50}
            height={50}
            src={student.picture ? student.picture : "/avatar.png"}
            alt={student.name}
          />
          {student.name}
        </th>
        ...
        <td className="px-6 py-4">{student.medium}</td>
        <td className="px-6 py-4">{student.rn}</td>
        <td className="px-1 py-4">
          <div ref={ref} className="relative">
            <div
              onClick={() => setShowMenu((prev) => !prev)}
              className="hover:bg-gray-100 w-8 h-8 flex justify-center active:bg-gray-300 hover:shadow items-center text-black rounded-full cursor-pointer"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
            ...
            >
              <div
                ...
                className="flex space-x-3 px-3 py-2 hover:bg-gray-100 cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <button
                ...
                className="flex disabled:cursor-not-allowed disabled:bg-gray-100 space-x-3 px-3 py-2 hover:bg-gray-100 cursor-pointer"
              >
                <div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
