...
const fees = () => {
  const { employee, classes, setFees, fees, total, setTotal } =
  ...
  const fetchFees = async (ignoreDates) => {
    ...
    try {
      if (
        ...
        res = await axios.get(
          `/api/studentfee/get?campus=${localStorage.getItem(
            "campus"
          )}&startedDate=${filters.startedDate}&endedDate=${filters.endedDate}&items=${items}`
        );
      } else if (fClass && fSession && !ignoreDates) {
        res = await axios.get(
          `/api/studentfee/get?campus=${localStorage.getItem(
            "campus"
          )}&class=${fClass}&session=${fSession}&items=${items}`
        );
      } else {
        res = await axios.get(
          `/api/studentfee/get?campus=${localStorage.getItem("campus")}&items=${items}`
        );
      }
  ...
  const fetchMore = async () => {
    ...
    try {
      ...
      if (filters.startedDate !== "" && filters.endedDate !== "") {
        res = await axios.get(
          `/api/studentfee/get?page=${currentPage}&campus=${localStorage.getItem(
            "campus"
          )}&startedDate=${filters.startedDate}&endedDate=${
            filters.endedDate
          }&items=${items}`
        );
  ...
  return (
    <div>
      ...
      <main>
          ...
          <div className="flex w-full lg:w-auto lg:flex-1 flex-col p-4 md:p-5 gap-2">
            <div className="flex">
              <div className="flex justify-between items-center w-full">
                <button
                  onClick={() => setShowCreateMultipleFees(true)}
                  className="base__button px-3 gap-2"
                >
                  <span>
                    <svg
              ...
              <div className="mx-auto max-w-screen-xl">
                <div className="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
                  <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                      <div className="flex gap-3 items-center flex-wrap">
                        <input
                          ...
                          name="startedDate"
                          type="date"
                          className="base__input py-[5px] px-3 mt-0 border-gray-100 bg-white text-sm"
                        />
                        <input
                          onChange={handleChange}
                          value={filters.endedDate}
                          name="endedDate"
                          type="date"
                          className="base__input py-[5px] px-3 mt-0 border-gray-100 bg-white text-sm"
                        />
                        <button
                          className="base__button px-3 text-sm"
                        ...
                        {filters.startedDate !== "" &&
                          filters.endedDate !== "" && (
                            <div
                              onClick={handleClear}
                              className="flex p-1 bg-gray-100 rounded-full shadow cursor-pointer hover:scale-125 transition-all duration-300"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <select
                            ...
                            value={fClass}
                            className="base__select bg-white px-3 text-sm"
                          >
                    ...
                    <div className="">
                      <span className="text-sm">
                        Records: <b>{total.total ? total.total : "__"}</b>
                      </span>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-sm text-left text-gray-500 ">
                      <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                          <th
                            scope="col"
                            className="px-4 py-3 min-w-[30px] max-w-[30px]"
                          >
                            Srno.
                          </th>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td className="px-4 py-3 text-lg">
                            <b>
                              Rs.
                              {new Intl.NumberFormat().format(getFeeSum(fees))}
                            </b>
                          </td>
                        </tr>
                      </tbody>
                  ...
                  >
                    <button
                      onClick={fetchMore}
                      disabled={currentPage === total?.pages}
                      className="base__button"
                    >
                      Show more
                    </button>
                    <p className="text-gray-600">
                      Showing <b>{currentPage}</b> of <b>{total.pages}</b> pages
                    </p>
                  </nav>
                </div>
              </div>
        ...
        {showCreateMultipleFees && (
          <CreateMultipleFees
            setShowCreateMultipleFees={setShowCreateMultipleFees}
          />
        )}
      </main>
    </div>
...
const FeeRow = ({ fee, i, handleDelete }) => {
  ...
  return (
    <tr className="border-b animate-slow group">
      <th
        scope="row"
        className="px-4 py-3 font-medium text-gray-900 max-w-[30px]"
      >
        {i}
      </th>
      ...
      <td className="px-4 py-3">
        {new Intl.NumberFormat().format(fee.feeamount)}
      </td>
      <td className="px-4 py-3">{fee.remainings}</td>
      <td className="px-3 py-3 text-sm">
        {moment(fee?.createdAt).format("L")}
      </td>
      ...
      <td className="px-4 py-3 flex items-center justify-center">
        <div className="h-full flex items-center">
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6 scale-0 group-hover:scale-100 transition-all duration-200 cursor-pointer text-red-500"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
